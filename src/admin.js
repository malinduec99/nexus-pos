...
window.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        if (splash) {
            ...
            splash.style.pointerEvents = 'none'; // [MOD] Allow clicking through as soon as it starts fading
            setTimeout(() => splash.style.display = 'none', 500);
        }
    }, 800); // [MOD] Reduced from 2500ms to 800ms for faster startup
    ...
    const updateConnectivityStatus = () => {
        ...
        if (navigator.onLine) {
            ...
            // Only show toast if it was offline before to avoid annoying on load
            if (window._wasOffline) {
                window.showToast("☁️ Back Online: Syncing changes...", "success");
            ...
            window.showToast("🔌 Offline: Working on Local Buffer", "warning");
...
const CLIENT_VERSION = '1.5.3'; // Updated version for changes
window._adminProductLetterFilter = ''; // Globalization for alphabet filter

const THEMES = {
    original: { primary: '#38bdf8', accent: '#0ea5e9', bg: '#020617' },
    emerald: { primary: '#10b981', accent: '#34d399', bg: '#022c22' },
    crimson: { primary: '#ef4444', accent: '#f87171', bg: '#450a0a' },
    cyber: { primary: '#8b5cf6', accent: '#a78bfa', bg: '#0f172a' }
};
...
function showAdminActions() {
    ...
    onSnapshot(productsCol, (snapshot) => {
        ...
        requestAnimationFrame(() => {
            ...
            if (window.handlePOSSearch) window.handlePOSSearch(document.getElementById('pos-search')?.value || '');
    ...
    window.searchPartsForRepair = (val) => {
        ...
        const filtered = (window.allProducts || []).filter(p => 
            (p.name && p.name.toLowerCase().includes(q)) || (p.sku && p.sku.toLowerCase().includes(q))
        );
    ...
    document.getElementById('expense-form')?.addEventListener('submit', (e) => window.addExpense?.(e));
    document.getElementById('supplier-form')?.addEventListener('submit', (e) => window.addSupplier?.(e));
    document.getElementById('income-form')?.addEventListener('submit', (e) => window.addIncome?.(e));
    document.getElementById('customer-form')?.addEventListener('submit', (e) => window.addCustomer?.(e));
    document.getElementById('category-form')?.addEventListener('submit', (e) => window.addCategory?.(e));
    const productForm = document.getElementById('product-form');
    if (productForm) {
        productForm.onsubmit = async (e) => {
            e.preventDefault();
            if (window.addProduct) await window.addProduct(e);
        };
    }
    ...
    if (editProductForm) {
        editProductForm.onsubmit = async (e) => {
            e.preventDefault();
            if (window.updateProduct) await window.updateProduct(e);
        };
    }
    document.getElementById('bulk-upload-btn')?.addEventListener('click', (e) => window.handleBulkUpload?.(e));
    document.getElementById('branch-form')?.addEventListener('submit', (e) => window.addBranch?.(e));
    document.getElementById('store-form')?.addEventListener('submit', (e) => window.addStore?.(e));
...
function showTab(tabId) {
    ...
    if (tabId === 'pos') {
        ...
        // [MOD] Fast-focus and initialize POS search
        setTimeout(() => {
            const sInput = document.getElementById('pos-search');
            if (sInput) {
                sInput.focus();
                if (window.handlePOSSearch) window.handlePOSSearch(sInput.value);
            }
        }, 300);
    }
    ...
    if (tabId === 'credit') {
        titleEl.innerText = 'Credit Ledger (Naya Bill)';
        subEl.innerText = 'Track outstanding balances and customer credit history';
        window.renderCreditLedgerUI();
    }
}
...
function renderAdminProductsUI(products) {
    ...
    const sidebarDiv = document.getElementById('alphabet-sidebar');
    if (!listDiv) return;
    
    let filtered = window.filterByStore(products);
    
    // 1. Sort Alphabetically (A-Z) by default
    ...
    // 3. Alphabet Sidebar Filtering
    if (window._adminProductLetterFilter) {
        filtered = filtered.filter(p => (p.name || '').toUpperCase().startsWith(window._adminProductLetterFilter));
    }
    
    // 4. Render Alphabet Sidebar
    if (sidebarDiv) {
        const letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
        sidebarDiv.innerHTML = `
            <div class="alphabet-letter all ${!window._adminProductLetterFilter ? 'active' : ''}" onclick="window.setProductLetterFilter('')">ALL</div>
            ${letters.map(l => `
                <div class="alphabet-letter ${window._adminProductLetterFilter === l ? 'active' : ''}" onclick="window.setProductLetterFilter('${l}')">${l}</div>
            `).join('')}
        `;
    }

    if (filtered.length === 0) {
        listDiv.innerHTML = '<div style="padding: 3rem; text-align: center;">No products matching criteria.</div>';
        return;
    }
                    ...
                    <tr>
                        <td>
                            <div style="display: flex; align-items: center; gap: 1rem;">
                                ...
                                <div style="font-weight: 700; color: #38bdf8;">${p.name}</div>
                            </div>
                        </td>
                        <td style="font-family: monospace; font-weight: bold; color: #38bdf8;">${p.sku || '-'}</td>
                        <td>${p.category || '-'}</td>
                        <td style="font-weight:600; color: #cbd5e1;">LKR ${(p.cost || 0).toLocaleString()}</td>
                        <td style="font-weight:700; color: #38bdf8;">
                            <div>R: ${(p.price || 0).toLocaleString()}</div>
                            <div style="font-size:0.8em; color: #facc15;">W: ${(p.wholesalePrice || 0).toLocaleString()}</div>
                            <div style="font-size:0.8em; color: #10b981;">L: ${(p.loyaltyPrice || 0).toLocaleString()}</div>
                        </td>
                        ${(p.mode === 'Service' || p.mode === 'Technical')
                            ? `<td style="color: var(--primary-blue); font-weight:800;"> SERVICE (∞) </td>`
                            : `
                            <td style="font-weight:700;">
                                <div style="color: ${p.stock < 5 ? '#ef4444' : '#10b981'};">S: ${p.stock || 0}</div>
                                <div style="color: ${(p.wStock || 0) < 5 ? '#f59e0b' : '#38bdf8'}; font-size: 0.8em;">W: ${p.wStock || 0}</div>
                            </td>
                            `
                        }
...
window.renderAdminProductsUI = renderAdminProductsUI;

window.setProductLetterFilter = (letter) => {
    window._adminProductLetterFilter = letter;
...
function renderAdminOrdersUI(orders) {
    ...
    ordersDiv.innerHTML = filtered.length === 0 ? '<div style="text-align:center; padding:4rem; color: #94a3b8;">No orders found.</div>' : filtered.map(order => `
        <div class="order-card glass-premium" style="padding:1.5rem; margin-bottom:1rem; border:1px solid rgba(255,255,255,0.1);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:1rem;">
                <div>
                    <span style="font-weight: 800; color: #38bdf8;">#${order.orderId}</span>
                    <div style="font-size: 0.75rem; color: rgba(255,255,255,0.6);">${order.date}</div>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1.5rem;">
                <div>
                    <h5 style="margin:0 0 0.5rem 0; color:rgba(255,255,255,0.5); font-size:0.75rem;">CUSTOMER</h5>
                    <div style="font-weight:700; color: #fff;">${order.userName}</div>
                    <div style="font-size:0.85rem; color: rgba(255,255,255,0.8);">📞 ${order.phone}</div>
                    <div style="font-size:0.85rem; margin-top:0.3rem; color: rgba(255,255,255,0.7);">🏠 ${order.address}, ${order.city}</div>
                </div>
                <div>
                    <h5 style="margin:0 0 0.5rem 0; color:rgba(255,255,255,0.5); font-size:0.75rem;">BILLING</h5>
                    <div style="font-weight:800; color:#38bdf8; font-size:1.1rem;">LKR ${order.total.toFixed(2)}</div>
                    <div style="font-size:0.8rem; margin-bottom:0.5rem; color: rgba(255,255,255,0.8);">Method: ${order.paymentMethod}</div>
            ...
            <div style="margin-top:1.5rem; display:flex; gap:0.5rem; align-items:center;">
                ...
                <button onclick="printOrderInvoice('${order.docId}')" class="action-btn" style="padding:0.4rem 1rem; border-radius:8px; background:rgba(255,255,255,0.1); color:#38bdf8; border:none; font-size:0.8rem;">📄 Thermal</button>
                <button onclick="openModernInvoice('${order.docId}')" class="action-btn" style="padding:0.4rem 1rem; border-radius:8px; background:rgba(227,0,48,0.2); color:#ff4d6d; border:none; font-size:0.8rem; font-weight:bold;">📸 Graphic Invoice</button>
        ...
        if (newStatus !== 'Pending' && !order.stockDeducted) {
            if (confirm("Deduct items from stock?")) {
                for (const item of order.items) {
                    ...
                    if (product && !['Service', 'Technical'].includes(product.mode)) {
                        const prodRef = doc(db, 'products', product.docId);
                        const isPickup = (order.paymentMethod === 'Shop Pickup');
                        
                        if (isPickup) {
                            batch.update(prodRef, { stock: Math.max(0, (product.stock || 0) - item.quantity) });
                        } else {
                            // Default to Warehouse Stock for Delivery/COD/Bank
                            batch.update(prodRef, { wStock: Math.max(0, (product.wStock || 0) - item.quantity) });
                        }
                    }
                }
        ...
        try {
            ...
            if (order.items && Array.isArray(order.items)) {
                for (const item of order.items) {
                    ...
                    if (currentProd && !['Service', 'Technical'].includes(currentProd.mode)) {
                        const prodRef = doc(db, 'products', item.docId);
                        const isPickupOrPOS = (order.isPOS || order.paymentMethod === 'Shop Pickup');
                        
                        if (isPickupOrPOS) {
                            const newStock = (currentProd.stock || 0) + (item.quantity || 1);
                            batch.update(prodRef, { stock: newStock });
                        } else {
                            const newWStock = (currentProd.wStock || 0) + (item.quantity || 1);
                            batch.update(prodRef, { wStock: newWStock });
                        }
                    }
                }
            }
...
let scannerCallback = null; // [NEW] Track where the scan result should go

window.openScanner = (callback = null) => {
    scannerCallback = callback; // [NEW] Set callback if provided
...
const onScanSuccess = (decodedText) => {
    ...
    // [NEW] If a custom callback is provided, use it and close
    if (typeof scannerCallback === 'function') {
        scannerCallback(q);
        window.closeScanner();
        scannerCallback = null; // Clear it
        return;
    }

    const searchQ = q.toLowerCase();
    const product = window.allProducts.find(p => 
        (p.sku && p.sku.toLowerCase() === searchQ) ||
        p.name.toLowerCase() === searchQ
    );
        ...
        return `
            <div class="pos-v2-item" onclick="addToPOSBill('${p.docId}')">
                ...
                <div style="flex:1;">
                    ...
                    <div style="font-size:0.75rem; color:${(p.mode === 'Service' || p.mode === 'Technical') ? '#10b981' : (p.stock < 5 ? '#ef4444' : '#64748b')}; font-weight:700;">
                        Stock: ${(p.mode === 'Service' || p.mode === 'Technical') ? 'Unlimited' : p.stock}
                    </div>
                </div>
            </div>
        `;
...
window.openPaymentModal = () => {
    ...
    // [MOD] Show Customer Credit/Outstanding Info
    const customerPhoneInput = document.getElementById('pos-cust-phone');
    const customerPhone = customerPhoneInput ? customerPhoneInput.value.trim().toLowerCase() : "";
    const customer = (window.allCustomers || []).find(c => 
        (c.phone && c.phone.toLowerCase() === customerPhone) || 
    ...
    const outstandingWrap = document.getElementById('pay-modal-outstanding-wrap');
    const outstandingVal = document.getElementById('pay-modal-outstanding');
    if (customer) {
        const outstanding = window.calculateCustomerOutstanding(customer.docId);
        if (outstandingVal) outstandingVal.innerText = outstanding.toFixed(2);
        if (outstandingWrap) outstandingWrap.style.display = 'block';
    } else {
        if (outstandingWrap) outstandingWrap.style.display = 'none';
    }
...
window.processPOSSale = async (method) => {
    ...
    // [MOD] Credit bill REQUIRE Customer Details
    if (method === 'Credit' && !customer) {
        window.showToast("⚠️ Customer details required for Credit bills!", "error");
        document.getElementById('pay-modal-tendered').blur();
...
window.calculateCustomerOutstanding = (customerId) => {
    const customer = window.allCustomers?.find(c => c.docId === customerId);
    if (customer && typeof customer.outstanding === 'number') return customer.outstanding;
    
    if (!window.allOrders || !customerId) return 0;
    const storeId = window.getStoreId();
    return window.allOrders.filter(o =>
        o.customerId === customerId &&
        o.paymentMethod === 'Credit' &&
        (o.storeId === storeId || storeId === 'master')
    ).reduce((sum, o) => sum + (o.total || 0), 0);
};

window.renderCreditLedgerUI = () => {
    const list = document.getElementById('credit-ledger-body');
    const totalEl = document.getElementById('credit-total-outstanding');
    if (!list || !window.allOrders || !window.allCustomers) return;

    const searchTerm = document.getElementById('credit-search-input').value.toLowerCase().trim();
    const storeId = window.getStoreId();

    const creditOrders = window.allOrders.filter(o => o.paymentMethod === 'Credit' && (o.storeId === storeId || storeId === 'master'));

    const customersWithCredit = window.allCustomers.map(c => {
        const myOrders = creditOrders.filter(o => o.customerId === c.docId);
        const outstanding = myOrders.reduce((sum, o) => sum + (o.total || 0), 0);
        return { ...c, creditCount: myOrders.length, outstanding };
    }).filter(c => c.outstanding > 0);

    const filtered = customersWithCredit.filter(c =>
        c.name.toLowerCase().includes(searchTerm) ||
        c.phone.toLowerCase().includes(searchTerm)
    );

    let totalAll = filtered.reduce((sum, c) => sum + c.outstanding, 0);
    if (totalEl) totalEl.innerText = `LKR ${totalAll.toFixed(2)}`;

    list.innerHTML = filtered.map(c => `
        <tr>
            <td style="font-weight:bold;">${c.name}</td>
            <td>${c.phone}</td>
            <td>${c.creditCount} Bills</td>
            <td style="font-weight:900; color:#e11d48;">LKR ${c.outstanding.toFixed(2)}</td>
            <td style="text-align:right;">
                <button onclick="window.viewCustomerCreditDetail('${c.docId}')" class="action-btn" style="background:#0ea5e9; color:white; border:none; padding:8px 15px; border-radius:8px; font-size:0.8rem; cursor:pointer;">📂 View Bills</button>
            </td>
        </tr>
    `).join('');
};

window.viewCustomerCreditDetail = (cid) => {
    const cust = window.allCustomers.find(c => c.docId === cid);
    if (!cust) return;

    window.showTab('orders');
    const searchInput = document.getElementById('order-search-input');
    if(searchInput) {
        searchInput.value = cust.phone;
        window.applyOrderFilters();
        window.showToast(`Showing Credit Bills for ${cust.name}`, "info");
    }
};

window.completeAndSharePOSSale = async (shareType) => {
    ...
    try {
        ...
        } else if (shareType === 'Graphic') {
            window.lastSavedPOSOrder = currentPendingPOSSale;
            window.openModernInvoice(currentPendingPOSSale.orderId);
        }

        // 2. Save to Firebase (using atomic batch for consistency)
        const batch = writeBatch(db);
        const orderRef = doc(ordersCol); // Auto-generate ID
        batch.set(orderRef, currentPendingPOSSale);
        
        // Log the sale logic (non-blocking)
        ...
        for (const item of currentPendingPOSSale.items) {
            if (item.docId && item.docId !== 'service_charge' && !['Service', 'Technical'].includes(item.mode)) {
                const prodRef = doc(db, 'products', item.docId);
        ...
        // 3. Update Customer Data (Loyalty + Credit Outstanding)
        if (currentPendingPOSSale.customerId) {
            ...
            updatePayload.loyaltyPoints = Math.max(0, newPoints);

            // --- CREDIT OUTSTANDING [NEW DIRECT PERSISTENCE] ---
            if (currentPendingPOSSale.paymentMethod === 'Credit') {
                const currentOutstanding = customer?.outstanding || 0;
                updatePayload.outstanding = currentOutstanding + currentPendingPOSSale.total;
            }
        ...
        window.showToast("✅ Sale Recorded & Synced to Firestore!", "success");
        const sound = document.getElementById('notif-sound-pos');
        if (sound) sound.play().catch(e => console.log(e));
        ...
        window.showToast("❌ Error completing sale! Check console.", "error");
    }
};
...
window.addIncome = async (e) => {
    ...
    try {
        ...
        console.error(err);
        window.showToast("❌ Error saving income", "error");
    }
};
...
window.addVariantField = () => {
    ...
    div.style = "display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 1fr 1fr 1fr 1.5fr 40px auto; gap: 10px; margin-bottom: 10px; align-items: end;";
    div.innerHTML = `
        ...
        <div class="input-group"><label>Wholesale</label><input type="number" class="v-wholesale"></div>
        <div class="input-group"><label>Loyalty</label><input type="number" class="v-loyalty"></div>
        ...
        <div class="input-group"><label>Image</label><input type="file" onchange="window.previewVariantImage(this, 'v-prev-${id}')"></div>
...
window.toggleStockVisibility = (mode, stockInputId) => {
    const stockInput = document.getElementById(stockInputId);
    if (!stockInput) return;
    const group = stockInput.closest('.input-group');
    
    const wStockId = stockInputId.replace('-stock', '-wstock');
    const wStockInput = document.getElementById(wStockId);
    const wGroup = wStockInput?.closest('.input-group');

    const isEdit = stockInputId.includes('edit');
    const payBoxId = isEdit ? 'edit-p-pay-methods-box' : 'p-pay-methods-box';
    const payBox = document.getElementById(payBoxId);

    if (mode === 'Service' || mode === 'Technical') {
        if (group) group.style.display = 'none';
        if (wGroup) wGroup.style.display = 'none';
        if (payBox) payBox.style.display = 'none';
        stockInput.value = 999999;
        if (wStockInput) wStockInput.value = 999999;
    } else {
        if (group) group.style.display = 'block';
        if (wGroup) wGroup.style.display = 'block';
        if (payBox) payBox.style.display = 'block';
        if (stockInput.value === '999999') stockInput.value = 0;
        if (wStockInput && wStockInput.value === '999999') wStockInput.value = 0;
    }
};
...
window.openAddProductModal = () => {
    ...
    if (modeSelect) {
        modeSelect.value = 'Shop';
        window.toggleStockVisibility('Shop', 'p-stock');
    }
};
...
window.addProduct = async function(e) {
    ...
    // Safety: ensure it is a form submission or we have the form reference
    const form = e?.target || document.getElementById('product-form');
    if (!form) {
        console.error("Product form not found during submission");
        return;
    }

    if (window._isPublishing) {
        window.showToast("⏳ Please wait, already publishing...", "warning");
        return;
    }
    const submitBtn = form.querySelector('button[type="submit"]');
    
    try {
        window._isPublishing = true;
        if (submitBtn) {
            submitBtn.disabled = true;
            submitBtn.innerText = "⌛ Publishing...";
        }
        window.showToast("🚀 Publishing product...", "info");

        const name = document.getElementById('p-name')?.value || 'Unnamed';
        const category = document.getElementById('p-category')?.value || '';
        const mode = document.getElementById('p-mode')?.value || 'Shop';
        const hasVariants = document.getElementById('p-has-variants')?.checked || false;
        ...
        let productData = {
            ...
            storeId: window.getStoreId(),
            timestamp: serverTimestamp(),
            payMethods: (mode === 'Service' || mode === 'Technical') ? 
                { cod: true, bank: true, shop: true } : 
                {
                    cod: document.getElementById('p-pay-cod')?.checked ?? true,
                    bank: document.getElementById('p-pay-bank')?.checked ?? true,
                    shop: document.getElementById('p-pay-shop')?.checked ?? true
                }
        };
        ...
        if (hasVariants) {
            ...
            for (const row of rows) {
                ...
                let vImageUrl = "";
                if (vImageFile) vImageUrl = await window.toBase64(vImageFile);
                ...
                variants.push({
                    ...
                    wholesalePrice: parseFloat(row.querySelector('.v-wholesale')?.value) || 0,
                    loyaltyPrice: parseFloat(row.querySelector('.v-loyalty')?.value) || 0,
            ...
            if (variants.length > 0) {
                ...
                productData.cost = first.cost;
                productData.price = first.price;
                productData.wholesalePrice = first.wholesalePrice;
                productData.loyaltyPrice = first.loyaltyPrice;
                productData.discount = first.discount;
            ...
            productData.discount = parseFloat(document.getElementById('p-discount')?.value) || 0;
            
            if (mode === 'Service' || mode === 'Technical') {
                productData.stock = 999999;
                productData.wStock = 999999;
            } else {
                productData.stock = parseInt(document.getElementById('p-stock')?.value) || 0;
                productData.wStock = parseInt(document.getElementById('p-wstock')?.value) || 0;
            }
            
            const imageFile = document.getElementById('p-image')?.files[0];
            if (imageFile) productData.image = await window.toBase64(imageFile);
        }
        ...
        window._isPublishing = false;
        if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.innerText = "Create Product & Publish";
        }
    }
};
...
window.openEditModal = async (docId) => {
    ...
    document.getElementById('edit-p-wstock').value = product.wStock || 0;
    ...
    window.toggleStockVisibility(product.mode || 'Shop', 'edit-p-stock');
...
window.addEditVariantField = (data = null) => {
    ...
    div.innerHTML = `
        ...
        <div class="input-group"><label>Wholesale</label><input type="number" class="v-wholesale" value="${data ? data.wholesalePrice : 0}"></div>
        <div class="input-group"><label>Loyalty</label><input type="number" class="v-loyalty" value="${data ? data.loyaltyPrice : 0}"></div>
        ...
        <div class="input-group"><label>Image</label><input type="file" onchange="window.previewVariantImage(this, 'ev-prev-${id}')"></div>
...
window.updateProduct = async function(e) {
    ...
    try {
        ...
        const mode = document.getElementById('edit-p-mode')?.value || 'Shop';
        ...
        if (hasVariants) {
            ...
            if (variants.length > 0) {
                const first = variants[0];
                updates.cost = first.cost;
                updates.price = first.price;
                updates.wholesalePrice = first.wholesalePrice;
                updates.loyaltyPrice = first.loyaltyPrice;
                updates.discount = first.discount;
                updates.stock = variants.reduce((sum, v) => sum + v.stock, 0);
                updates.image = first.image;
            }
        } else {
            ...
            if (mode === 'Service' || mode === 'Technical') {
                updates.stock = 999999;
                updates.wStock = 999999;
            } else {
                updates.stock = parseInt(document.getElementById('edit-p-stock')?.value) || 0;
                updates.wStock = parseInt(document.getElementById('edit-p-wstock')?.value) || 0;
            }
            
            const imageFile = document.getElementById('edit-p-image')?.files[0];
            if (imageFile) updates.image = await window.toBase64(imageFile);
        }

        window.showToast("🚀 Updating product...", "info");
...
window.renderCategoriesUI = (categories) => {
    ...
    list.innerHTML = filtered.map(c => `
        <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(255, 255, 255, 0.05); padding: 10px 15px; border-radius: 12px; margin-bottom: 8px; border: 1px solid rgba(255, 255, 255, 0.1);">
...
window.downloadProductsCSV = () => {
    const products = window.allProducts || [];
    let csv = "Name,ShortCode(SKU),Category,Mode,Cost,RetailPrice,WholesalePrice,LoyaltyPrice,Discount(%),Stock,WarehouseStock\n";
    products.forEach(p => {
        const name = (p.name || '').replace(/"/g, '""');
        const sku = (p.sku || '').replace(/"/g, '""');
        const cat = (p.category || '').replace(/"/g, '""');
        const mode = (p.mode || 'Shop');
        const cost = p.cost || 0;
        const price = p.price || 0;
        const wholesale = p.wholesalePrice || 0;
        const loyalty = p.loyaltyPrice || 0;
        const disc = p.discount || 0;
        const stock = p.stock || 0;
        const wStock = p.wStock || 0;
        csv += `"${name}","${sku}","${cat}","${mode}",${cost},${price},${wholesale},${loyalty},${disc},${stock},${wStock}\n`;
    });
    downloadFile(csv, "MEC_Products_Full_Export.csv");
};

window.downloadStockPDF = () => {
    const products = window.allProducts || [];
    const now = new Date();
    ...
    const rows = products.map((p, idx) => {
        const stock = (p.mode === 'Service' || p.mode === 'Technical') ? '∞' : (p.stock || 0);
        const costVal = (p.mode === 'Service' || p.mode === 'Technical') ? 0 : (p.stock || 0) * (p.cost || 0);
        const retailVal = (p.mode === 'Service' || p.mode === 'Technical') ? 0 : (p.stock || 0) * (p.price || 0);
        ...
        return `
            <tr>
                ...
                <td>${p.category || '-'}</td>
                <td style="text-align:right;">${(p.cost || 0).toFixed(2)}</td>
                <td style="text-align:right;">${(p.price || 0).toFixed(2)}</td>
                <td style="text-align:center;">${stock}</td>
                <td style="text-align:right;">${costVal.toFixed(2)}</td>
            </tr>
        `;
    }).join('');
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Stock Report - MEC POS</title>
            <style>
                body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 40px; color: #333; }
                .header { text-align: center; border-bottom: 2px solid #002060; padding-bottom: 20px; margin-bottom: 30px; }
                .header h1 { margin: 0; color: #002060; font-size: 28px; }
                .header p { margin: 5px 0; color: #666; }
                table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
                th { background: #f8fafc; color: #002060; font-weight: bold; text-transform: uppercase; font-size: 12px; border: 1px solid #e2e8f0; padding: 12px 8px; }
                td { border: 1px solid #e2e8f0; padding: 10px 8px; font-size: 13px; vertical-align: middle; }
                tr:nth-child(even) { background: #fcfdfe; }
                .summary { display: flex; justify-content: flex-end; gap: 40px; margin-top: 40px; padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; }
                .summary-item { text-align: right; }
                .summary-label { font-size: 11px; color: #64748b; font-weight: bold; text-transform: uppercase; }
                .summary-value { font-size: 20px; color: #002060; font-weight: 800; }
                @media print {
        ...
        <body>
            <div class="header">
                <h1>MEC BOOK SHOP - INVENTORY REPORT</h1>
                <p>System Generated: ${dateStr}</p>
                <p>Managed by MEC POS Security Platform</p>
            </div>
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="text-align:right;">Cost (LKR)</th>
            ...
            <div class="summary">
                ...
                <div class="summary-item">
                    <div class="summary-label">Total Cost Valuation</div>
                    <div class="summary-value">LKR ${totalCostValuation.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                </div>
                <div class="summary-item">
                    <div class="summary-label">Total Retail Valuation</div>
                    <div class="summary-value">LKR ${totalRetailValuation.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                </div>
            </div>
            <div class="no-print" style="margin-top: 30px; text-align: center;">
                <button onclick="window.print()" style="padding: 12px 24px; background: #002060; color: white; border: none; border-radius: 6px; font-weight: bold; cursor: pointer;">🖨️ Print / Save as PDF</button>
            </div>
        </body>
        </html>
    `);
...
const saveRepairJob = async (jobId, name, phone, desc, issue, cost, advance, balance) => {
    try {
        ...
        window.showToast("✅ Repair job recorded!", "success");
    } catch (e) {
        console.error("Error saving repair job: ", e);
        window.showToast("❌ Failed to save repair job", "error");
    }
};
...
document.getElementById('employee-form')?.addEventListener('submit', async (e) => {
    ...
    try {
        window.showToast(navigator.onLine ? "🚀 Syncing staff details..." : "💾 Saving staff record locally...", "info");
        if (window.editingEmployeeId) {
            await updateDoc(doc(db, 'employees', window.editingEmployeeId), staffData);
            window.showToast(navigator.onLine ? "✅ Staff Details Updated!" : "✅ Saved Locally", "success");
            window.editingEmployeeId = null;
            const submitBtn = document.querySelector('#employee-form button[type="submit"]');
            if (submitBtn) submitBtn.innerText = "Register Employee";
        } else {
            await addDoc(employeesCol, staffData);
            window.showToast(navigator.onLine ? "✅ Staff Registered Successfully!" : "✅ Registered Locally", "success");
        }
        e.target.reset();
...
document.getElementById('payroll-form')?.addEventListener('submit', async (e) => {
    ...
    e.target.reset();
    window.showToast(navigator.onLine ? "✅ Salary Payment Recorded & Linked to Expenses!" : "✅ Salary Saved Locally & Linked", "success");
});
